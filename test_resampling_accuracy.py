...
fake_threshold = np.linspace(0.18, 0.23, 200)
#fake_threshold = 0.2
...
for i in range(0, np.size(fake_threshold)):
    temp_accuracy = fake_image_detection(fake_threshold=fake_threshold[i], num_of_images=300, show_bar_chart=False, show_point_plot=False, show_truth_table=False, produce_half_sample=True)
